...
(function(){
  ...
  /* Attache le détail des moyens de paiement (paiement mixte) à une vente.
     Best-effort : n'échoue jamais la vente si ça ne passe pas. */
  function setPaiements(venteId, paiements){
    if (!ready() || !venteId) return Promise.resolve(false);
    return client.rpc('set_vente_paiements', { p_vente_id: venteId, p_paiements: paiements })
